"""
Модуль с консольным пользовательским интерфейсом для игры "Виселица".

Обеспечивает взаимодействие с игроком через командную строку:
вывод приветствия, отображение состояния игры, обработку ввода
и отрисовку ASCII-графики виселицы.

Пример:
    engine = GallowEngine("кот")
    ui = ConsoleUI(engine)
    ui.run()  # Запускает игровой цикл
"""
...
class ConsoleUI:
    ...
    def _greeting(self) -> None:
        ...
        print(
            f"всего у тебя будет {Config.MAX_ATTEMPTS} попыток, \n"
            "это будет видно по состоянию виселицы",
            end="\n\n",
        )
        print(
